...
第一期建立独立的用户数据服务，采用 Django、Django REST Framework 和 PostgreSQL。它只承载账号、资料和会话，后续再增加社区内容；本地赛事数据不上传到该服务，也不依赖它才能运行。本地自由/赛事模式、计分、复盘和导出均以 Electron Main 的 SQLite 为准，后续能力见 [后续目标](./FUTURE_GOALS_zh.md)。
