...
| Typed IPC | 部分完成 | 应用窗口、快捷键、Overlay、窗口枚举、设备扫描、legacy 项目列表/删除、报表和复盘已定义共享契约。 |
...
| 设备扫描 | 已切换扫描入口 | Renderer 经 IPC 调用 Worker 扫描 BLE/USB；连接、计分和 Reset 尚未切换。 |
...
扫描由 Platform Worker 执行，正式连接和实时计分仍由 legacy backend 执行。两套进程都包含 BLE/USB 能力，可能出现缓存不一致、重复扫描、端口占用和重连状态分裂。设备连接切换完成前必须保留实机回归和清晰的唯一所有者规则。
